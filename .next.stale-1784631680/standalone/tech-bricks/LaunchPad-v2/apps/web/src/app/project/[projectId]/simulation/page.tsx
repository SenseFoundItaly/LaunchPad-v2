...
function PersonaCard({ persona }: { persona: SimulatedPersona }) {
  ...
  const sentimentColor = {
    positive: 'bg-moss-wash text-moss',
    neutral: 'bg-accent-wash text-accent',
    negative: 'bg-clay-wash text-clay',
  }[persona.sentiment];

  const roleColor = {
    customer: 'text-sky',
    investor: 'text-plum',
    expert: 'text-cat-teal',
    competitor: 'text-cat-gold',
  ...
  return (
    <div className="bg-paper-2 border border-line rounded-xl p-4">
      ...
      {persona.feedback.length > 200 && (
        <button onClick={() => setExpanded(!expanded)} className="text-xs text-sky mt-1">
    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-ink">Stress Test</h3>
          <button
          ...
          <>
            ...
            <div className="space-y-3">
              {simulation.risk_scenarios.map((risk) => (
                <div key={risk.title} className="bg-paper-2 border border-line rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <h5 className="text-ink font-medium text-sm">{risk.title}</h5>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      risk.probability === 'high' ? 'bg-clay-wash text-clay' :
                      risk.probability === 'medium' ? 'bg-accent-wash text-accent' :
                      'bg-moss-wash text-moss'
                    }`}>P: {risk.probability}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      risk.impact === 'high' ? 'bg-clay-wash text-clay' :
                      risk.impact === 'medium' ? 'bg-accent-wash text-accent' :
                      'bg-moss-wash text-moss'
                    }`}>I: {risk.impact}</span>
                  </div>
                  <p className="text-xs text-ink-4 mb-2">{risk.description}</p>
                  <p className="text-xs text-sky">Mitigation: {risk.mitigation}</p>
                </div>
              ))}
            </div>
          </>
        ...
        {!simulation && !isRunning && !loading && (
          <div className="text-center py-20 text-ink-5">
            <p>Stress-test your idea against simulated customers, investors, and market conditions.</p>
          </div>
        )}
      </div>
    </div>
