module.exports=[824675,e=>{"use strict";var t=e.i(254799),a=e.i(921284);async function i(e){try{let i=t.default.randomUUID(),r=void 0===e.payload?null:e.payload;return await (0,a.run)(`INSERT INTO memory_events (id, user_id, project_id, event_type, payload)
       VALUES (?, ?, ?, ?, ?)`,i,e.userId,e.projectId,e.eventType,r),i}catch(e){return console.warn("[memory/events] recordEvent failed:",e),""}}async function r(e,t,i={}){let{limit:s=20,since:n,eventTypes:o}=i,l=["user_id = ?","project_id = ?"],c=[e,t];n&&(l.push("created_at >= ?"),c.push(n)),o&&o.length>0&&(l.push(`event_type IN (${o.map(()=>"?").join(",")})`),c.push(...o));let d=`SELECT id, user_id, project_id, event_type, payload, created_at
               FROM memory_events
               WHERE ${l.join(" AND ")}
...
Extract assumptions. Return JSON only.${"it"===l?'\n\nWrite each assumption\'s "text" value in Italian. Keep the JSON keys and the category/criticality enum values in English.':""}`,d=Date.now(),u=await (0,s.runAgent)(c,{systemPrompt:v,task:"assumption-extract",tools:!1,timeout:6e4,maxToolCalls:0});await (0,n.recordAgentUsage)({project_id:e,step:"assumption-extract",task:"assumption-extract",usage:u.usage,latency_ms:Date.now()-d});let p=function(e){let t=e.replace(/```json\s*/gi,"").replace(/```\s*$/g,"").trim(),a=t.indexOf("{"),i=t.lastIndexOf("}");if(-1===a||-1===i||i<=a)return null;let r=t.slice(a,i+1);try{let e=JSON.parse(r);if(!e||!Array.isArray(e.assumptions))return null;return e.assumptions.filter(e=>"object"==typeof e&&null!==e&&"string"==typeof e.text&&"string"==typeof e.category&&"string"==typeof e.criticality)}catch{return null}}(u.text);if(!p)return a.errors.push("Extractor returned non-JSON output"),a;let m=await (0,i.get)("SELECT MAX(number) AS max FROM assumptions WHERE project_id = ?",e),_=(m?.max??0)+1;for(let t of p){if(!f.has(t.category)||!E.has(t.criticality)||await (0,i.get)("SELECT id FROM assumptions WHERE project_id = ? AND LOWER(text) = LOWER(?) LIMIT 1",e,t.text)){a.skipped++;continue}try{await (0,i.run)(`INSERT INTO assumptions
          (id, project_id, number, category, text, source, explicit, criticality, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open')
         ON CONFLICT (project_id, number) DO NOTHING`,(0,r.generateId)("asm"),e,_,t.category,t.text,t.source??null,!0===t.explicit,t.criticality),a.inserted++,_++}catch(e){a.errors.push(e.message)}}return a}async function d(e,t){try{if(t.trim().length<40||await (0,i.get)("SELECT 1 AS one FROM assumptions WHERE project_id = ? LIMIT 1",e))return;c(e,t).catch(t=>{console.warn(`[assumptions] background seed failed for ${e}:`,t.message)})}catch(e){console.warn("[assumptions] seed guard failed (non-fatal):",e.message)}}async function u(e,t={}){let a=["project_id = ?"],r=[e];return t.status&&(Array.isArray(t.status)?(a.push(`status IN (${t.status.map(()=>"?").join(",")})`),r.push(...t.status)):(a.push("status = ?"),r.push(t.status))),t.criticality&&(a.push("criticality = ?"),r.push(t.criticality)),t.category&&(a.push("category = ?"),r.push(t.category)),(0,i.query)(`SELECT * FROM assumptions WHERE ${a.join(" AND ")} ORDER BY number ASC`,...r)}async function p(e){return(0,i.get)("SELECT * FROM assumptions WHERE id = ?",e)}async function m(e){let t=await (0,i.get)(`SELECT
       COUNT(*) AS total,
     ...
     WHERE id = ?`,t,a,e)}async function y(e,t){await (0,i.run)(`UPDATE assumptions
