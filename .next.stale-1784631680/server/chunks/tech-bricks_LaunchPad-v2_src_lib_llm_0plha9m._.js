module.exports=[302541,e=>{"use strict";e.s(["MODEL_CONFIG",0,{"claude-haiku-4-5":{id:"claude-haiku-4-5-20251001",openrouterId:"anthropic/claude-haiku-4.5",tier:"cheap",contextWindow:2e5,maxOutputTokens:64e3,pricing:{input:1,output:5,cacheWrite:1.25,cacheRead:.1}},"claude-sonnet-4-6":{id:"claude-sonnet-4-6",openrouterId:"anthropic/claude-sonnet-4.6",tier:"balanced",contextWindow:1e6,maxOutputTokens:64e3,pricing:{input:3,output:15,cacheWrite:3.75,cacheRead:.3}},"claude-opus-4-7":{id:"claude-opus-4-7",openrouterId:"anthropic/claude-opus-4.7",tier:"premium",contextWindow:1e6,maxOutputTokens:128e3,pricing:{input:5,output:25,cacheWrite:6.25,cacheRead:.5}}},"TIER_DEFAULTS",0,{cheap:{maxTokens:4096,temperature:.7},balanced:{maxTokens:8192,temperature:.7},premium:{maxTokens:16384,temperature:.7}}])},795686,e=>{"use strict";e.i(521131);var t=e.i(738539);e.i(985205);var i=e.i(723032),n=e.i(530178),a=e.i(516523),o=e.i(839205);let r=null,s=null,c=null,d="https://openrouter.ai/api/v1";async function u(e,t,i){let{provider:r,model:s,maxTokens:c}=(0,n.pickModel)(t),d=Date.now(),{text:u,usage:p}=await l(e,r,i.temperature??.3,c,s,i.userKey),m=Date.now()-d;if(i.projectId){let e="number"==typeof p.cost_usd?p.cost_usd:(0,o.estimateCost)(r,s,{input_tokens:p.input_tokens,output_tokens:p.output_tokens,cache_creation_input_tokens:p.cache_creation_input_tokens,cache_read_input_tokens:p.cache_read_input_tokens});(0,a.recordUsage)({project_id:i.projectId,step:t,provider:r,model:s,usage:{input:p.input_tokens,output:p.output_tokens,cacheCreation:p.cache_creation_input_tokens,cacheRead:p.cache_read_input_tokens,cost:{total:e}},latency_ms:m,...i.userKey?{key_source:"user"}:{}}).catch(e=>console.warn(`[${t}] recordUsage failed:`,e.message))}let h=u.trim();return JSON.parse((h=(h=h.replace(/^```(?:json)?\s*/,"").replace(/\s*```$/,"")).replace(/<think>[\s\S]*?<\/think>/g,"")).trim())}async function l(e,n="openai",a=.7,o=4096,u,p){let m=p?.provider==="anthropic"?p.apiKey:void 0,h=p?.provider==="openai"?p.apiKey:void 0,g=p?.provider==="openrouter"?p.apiKey:void 0;if("anthropic"===n){let t=e.filter(e=>"system"===e.role).map(e=>e.content).join("\n"),n=e.filter(e=>"system"!==e.role),r=await (m?new i.default({apiKey:m}):(!s&&(s=new i.default({apiKey:process.env.ANTHROPIC_API_KEY||"unused"})),s)).messages.create({model:u||process.env.ANTHROPIC_MODEL||"claude-sonnet-4-20250514",system:t,messages:n,temperature:a,max_tokens:o}),c="text"===r.content[0].type?r.content[0].text:"",d=r.usage;return{text:c,usage:{input_tokens:d.input_tokens??0,output_tokens:d.output_tokens??0,cache_creation_input_tokens:d.cache_creation_input_tokens??0,cache_read_input_tokens:d.cache_read_input_tokens??0}}}let _="openrouter"===n||p?.provider==="openrouter",y=_?g?new t.default({apiKey:g,baseURL:d}):(c||(c=new t.default({apiKey:process.env.OPENROUTER_API_KEY||"unused",baseURL:d})),c):h?new t.default({apiKey:h}):(r||(r=new t.default({apiKey:process.env.OPENAI_API_KEY||"unused"})),r),f=u||(_?process.env.OPENROUTER_MODEL||"anthropic/claude-sonnet-4.6":process.env.OPENAI_MODEL||"gpt-4o"),v=await y.chat.completions.create({model:f,messages:e,temperature:a,max_tokens:o}),k=v.choices[0].message.content||"",b=v.usage,w=b?.prompt_tokens_details?.cached_tokens??0,S=b?.cost;return{text:k,usage:{input_tokens:b?.prompt_tokens??0,output_tokens:b?.completion_tokens??0,cache_creation_input_tokens:0,cache_read_input_tokens:w,..."number"==typeof S?{cost_usd:S}:{}}}}e.s(["chatJSONByTask",0,u,"chatWithUsage",0,l])},727827,e=>{"use strict";let t=`You are a startup strategy consultant. Based on the startup idea, scoring,
...
}`,r=`You are an elite pitch coach who has helped founders raise over $1B in aggregate.
...
}`,s=`You are a startup attorney and fundraising advisor.
...
}`;e.s(["ANALYZE_PROMPT",0,n,"INVESTOR_UPDATE_PROMPT",0,c,"ITERATE_PROMPT",0,a,"PITCH_ITERATE_PROMPT",0,r,"STEP_SYSTEM_PROMPTS",0,i,"SYNTHESIZE_PROMPT",0,o,"TERM_SHEET_PROMPT",0,s,"WORKFLOW_PROMPT",0,t])}];

//# sourceMappingURL=tech-bricks_LaunchPad-v2_src_lib_llm_0plha9m._.js.map