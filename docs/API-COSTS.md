...
- **Chat is ~89% of all spend** ($74.55/30d as of 2026-08-10); of that, **74% is prompt-cache *writes*, not reads** — root cause + fix in §1 below (`CHAT_CACHE_SPLIT`, merged+deployed 2026-08-13, currently OFF — **not** the old `CACHE_PREFIX_SPLIT`, which is superseded/do-not-use).
...
- **The actual lever — `CHAT_CACHE_SPLIT` (`src/lib/chat/cache-breakpoint.ts` + `patches/@mariozechner+pi-ai+0.67.68.patch`).** Marks a cache breakpoint at the static/volatile boundary instead of moving any content — the model receives **byte-identical content in identical order** (asserted in `cache-breakpoint.test.ts` against the real prompt builder), so the Validation Gate risk above does not apply. Requires `npm ci` to actually apply the patch (it patches `openai-completions.js` in `node_modules` — see the patch file for why). **Status: merged + deployed to prod 2026-08-13 (`862fa44`), flag currently OFF (`CHAT_CACHE_SPLIT` unset on Netlify) — code is live but dark, zero effect on cost or behavior until the flag is set.** Measured in a controlled A/B (two fresh identical projects, real OpenRouter calls, split confirmed active in the provider log): writes −55%, input cost −50%. That measurement used short scripted non-streaming conversations with zero tool calls — direction and mechanism are solid (there's no code path by which flipping this can make things *worse* than today; every fallback strips the marker and reverts to current behavior), but the exact $ figure hasn't been confirmed against real streaming, tool-call-heavy traffic yet. Turning it on and watching `cache_read_tokens` in `llm_usage_logs WHERE step='chat'` for a few days is the way to find out.
...
2. **Flip `CHAT_CACHE_SPLIT`** (env var, Netlify) → biggest chat-margin lever, ~$25-30/mo modeled from a controlled A/B (writes −55%, chat input cost −50%); code shipped 2026-08-13, currently dark. Watch `cache_read_tokens` on `step='chat'` for a few days after flipping to confirm the number holds under real traffic. (`CACHE_PREFIX_SPLIT` — a different, older flag — is superseded and should stay OFF; it broke the Validation Gate 8/8/8→1/1/6 when tried.)
