    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-ink">Evidence Gathering</h3>
          <button
            onClick={runResearch}
            disabled={isRunning}
          ...
          <>
            ...
            <div className="bg-paper-2 border border-line rounded-xl p-6 mb-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  { label: 'TAM', value: research.market_size.tam, bg: 'bg-sky-wash', border: 'border-sky/20', text: 'text-sky' },
                  { label: 'SAM', value: research.market_size.sam, bg: 'bg-cat-teal-wash', border: 'border-cat-teal/20', text: 'text-cat-teal' },
                  { label: 'SOM', value: research.market_size.som, bg: 'bg-moss-wash', border: 'border-moss/20', text: 'text-moss' },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {research.competitors.map((comp) => (
                <div key={comp.name} className="bg-paper-2 border border-line rounded-xl p-4">
            ...
            <div className="space-y-3 mb-6">
              {research.trends.map((trend) => (
                <div key={trend.title} className="bg-paper-2 border border-line rounded-xl p-4">
                  <div className="flex items-center justify-between mb-1">
                    <h5 className="text-ink font-medium text-sm">{trend.title}</h5>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      trend.direction === 'growing' ? 'bg-moss-wash text-moss' :
                      trend.direction === 'declining' ? 'bg-clay-wash text-clay' :
                      'bg-ink-5/20 text-ink-4'
                    }`}>{trend.direction}</span>
                  </div>
                  <p className="text-xs text-ink-4">{trend.description}</p>
                  <p className="text-xs text-sky mt-1">Relevance: {trend.relevance}</p>
                </div>
              ))}
            </div>
            ...
            {research.key_insights && research.key_insights.length > 0 && (
              <>
                ...
                <div className="bg-sky-wash border border-sky/20 rounded-xl p-4 mb-6">
        ...
        {!research && !isRunning && !loading && (
          <div className="text-center py-20 text-ink-5">
            <p>Gather market evidence — competitors, trends, and blind spots you haven&apos;t considered.</p>
          </div>
        )}
      </div>
    </div>
