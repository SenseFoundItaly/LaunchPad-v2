...
export default function GrowthPage({
  ...
  function statusColor(status: GrowthIteration['status']) {
    switch (status) {
      case 'proposed':
        return 'text-sky bg-sky-wash';
      case 'testing':
        return 'text-accent bg-accent-wash';
      case 'tested':
        return 'text-ink-3 bg-ink-6/50';
      case 'adopted':
        return 'text-moss bg-moss-wash';
      case 'rejected':
        return 'text-clay bg-clay-wash';
      default:
        return 'text-ink-4 bg-paper-3';
    }
  }
    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-ink">Growth Experiments</h3>
          <button
        ...
        {activeLoop ? (
          <div>
            <button
              ...
              className="text-sm text-sky hover:text-sky mb-4 flex items-center gap-1 transition-colors"
            >
            ...
            <div className="bg-paper-2 border border-line rounded-xl p-6 mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={isRunning}
                    className="px-3 py-1.5 bg-paper-3 hover:bg-ink-6 disabled:bg-paper-3 text-ink-3 rounded-lg text-sm transition-colors"
                  >
                  ...
                  <button
                    ...
                    disabled={isRunning}
                    className="px-3 py-1.5 bg-moss hover:bg-moss/80 disabled:bg-paper-3 text-on-accent rounded-lg text-sm transition-colors"
                  >
              ...
              {activeLoop.accumulated_learnings && (
                <div className="bg-sky-wash border border-sky/30 rounded-lg p-4 mb-4">
                  <h5 className="text-xs font-medium text-sky mb-1">Accumulated Learnings</h5>
                ...
                >
                    ...
                    <div className="space-y-2 mb-3">
                      {iter.proposed_changes.map((change, i) => (
                        <div key={i} className="grid grid-cols-3 gap-2 text-xs">
                          ...
                          <div className="bg-clay-wash text-clay rounded px-2 py-1">
                            {change.current}
                          </div>
                          <div className="bg-moss-wash text-moss rounded px-2 py-1">
                            {change.proposed}
                          </div>
                        </div>
                      ))}
                  ...
                  {iter.status === 'proposed' && (
                    <button
                      ...
                      className="mt-3 px-3 py-1.5 bg-paper-3 hover:bg-ink-6 text-ink-3 rounded-lg text-xs transition-colors"
                    >
                      Log Result
                    </button>
                  )}
                  ...
                  {resultForm && resultForm.iterationId === iter.iteration_id && (
                    <div className="mt-3 bg-paper-3/50 rounded-lg p-4">
                      ...
                      <div className="flex justify-end gap-2 mt-3">
                        ...
                        <button
                          onClick={logResult}
                          className="px-3 py-1.5 bg-moss hover:bg-moss/80 text-on-accent rounded-lg text-xs font-medium transition-colors"
                        >
                          Save Result
                        </button>
                      </div>
            ...
            {loops.length > 0 ? (
              <div className="space-y-4">
                {loops.map((loop) => (
                  <button
                    ...
                    className="w-full text-left bg-paper-2 border border-line rounded-xl p-4 hover:border-line-2 transition-colors"
                  >
              ...
              <div className="text-center py-20 text-ink-5">
                ...
                <p className="text-sm mt-1">
                  Create a loop to run hypothesis-driven growth experiments with measurable outcomes.
                </p>
              </div>
            )}
          </>
