...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Grants tracking sync is NOT run here: a full three-source sync takes
    // ~75s (3 fetches + ~1,200 upserts), far past Netlify's 26s sync-function
    // limit. It streams from GET /api/cron/grants, which the scheduler calls
    // with curl -N after this endpoint returns (same pattern as run-monitor).
