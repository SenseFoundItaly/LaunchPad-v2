...
option-set: :::artifact{"type":"option-set","id":"opt_ID"}\n{"prompt":"?","options":[{"id":"a","label":"A","description":"..."},{"id":"run_x","label":"Run market research","description":"...","skill_id":"market-research"},{"id":"commit","label":"Confirm — commit to canvas","description":"Lock in this problem statement and move to Solution","commit":{"canvas":{"problem":"<exact agreed text>"}}}]}\n:::  (sources optional)
  COMMIT OPTION — set "commit" on an option to PERSIST evidence deterministically when the founder clicks it (the click = approval; never narrate a save instead). Two channels, combinable in one commit: ALL canvas CONTENT goes in "commit":{"canvas":{<field>:<value>, …}} (fields: problem | solution | target_market | value_proposition | business_model | competitive_advantage | channels | unfair_advantage | key_metrics | revenue_streams | cost_structure; the last three are LIST fields — pass a JSON array, one entry per item, e.g. "cost_structure":["Dati/cloud (fisso)","Sviluppo prodotto"]). Knowledge items go in "commit":{"items":[{"kind":"competitor"|"market_size_fact","name"?,"label","value","sources"}]} (competitor → graph, market size → fact). NEVER put a canvas field in items — canvas always goes in commit.canvas. Use the moment a value is settled.
...
idea-canvas: :::artifact{"type":"idea-canvas","id":"ic_ID"}\n{"title":"...","problem":"...","solution":"...","target_market":"...","value_proposition":"...","competitive_advantage":"...","unfair_advantage":"...","channels":"...","business_model":"...","key_metrics":["..."],"revenue_streams":["..."],"cost_structure":["..."]}\n:::  (sources optional — founder's own canvas)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { stream: piStream } = runAgentStream(effectiveLastMessage, {
      ...
      // 55s, NOT 180s (and not the first fix's 120s): the deployed function's
      // real ceiling is ~60s wall — re-measured live 2026-08-31 19:30Z on the
      // deployed runtime; the earlier ~110s bracket was contaminated by local
      // sim runs (see SKILL_RUN_BUDGET_MS in skill-executor.ts). A killed chat
      // turn persists NOTHING — the founder watches the answer stream, then
      // it's gone on refresh, and the credit was still charged. A 55s budget
      // abort keeps the streamed text and persists the message; a >60s turn
      // was ALREADY dying invisibly, so this cap trades an invisible loss for
      // a visible truncation. Also still cuts the agent-stuck-in-loop case —
      // pi-agent.ts force-closes the SSE controller at this deadline
      // regardless of whether agent.abort() propagates.
      timeout: 55000,
      task: chatTask,
