...
const proposeValidationTool = (ctx: ToolContext): AgentTool => ({
  ...
  async execute(_id, params): Promise<AgentToolResult<unknown>> {
    ...
    for (const it of rawItems) {
      ...
      if (it.kind === 'tech_fact' && !['feasibility', 'dependencies', 'regulatory'].includes(String(it.field))) {
        return { content: [{ type: 'text', text: 'tech_fact items require a "field": feasibility | dependencies | regulatory — it selects which 1B check the finding closes.' }], details: { error: true } };
      }
...
const logInterviewTool = (ctx: ToolContext): AgentTool => ({
  ...
  async execute(_id, params): Promise<AgentToolResult<unknown>> {
    ...
    // Freeze the pre-interview canvas on the first one logged — 1C's
    // "solution updated" / "value proposition sharpened" checks diff against
    // it. Chat is the primary logging path, so it must snapshot too or those
    // checks read a row nothing fills. Idempotent, non-fatal.
    await ensureCanvasBaseline(ctx.projectId);
