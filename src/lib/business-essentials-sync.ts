...
/**
 * Canvas → BUSINESS ESSENTIALS satellite sync.
 *
 ...
 * cost structure / key metrics). Names are STABLE so re-syncing after every
 * canvas write stays idempotent: the upsert lands on the expression unique
 * index (project_id, LOWER(name)) from migration 018 and just refreshes the
 * summary. AWAITED from the FOUNDER-GATED canvas write sites only (idea-canvas
 * POST, applyValidationProposal, context route) — never from the ungated chat
 * artifact path, and never throws into a caller.
 */
...
export async function syncBusinessEssentialNodes(projectId: string): Promise<void> {
  try {
    const row = await get<EssentialsRow>(
      'SELECT business_model, revenue_streams, cost_structure, key_metrics FROM idea_canvas WHERE project_id = ?',
      projectId,
    );
    if (!row) return;
    ...
    for (const s of populated) {
      ...
      const rows = await run(
        `INSERT INTO graph_nodes (id, project_id, name, node_type, summary, attributes, sources, reviewed_state)
         VALUES (?, ?, ?, 'business_essential', ?, ?, ?, 'applied')
         ...
         RETURNING id`,
        generateId('gnode'), projectId, s.name, s.summary,
      ...
      const existingEdge = await get<{ id: string }>(
        `SELECT id FROM graph_edges
          WHERE project_id = ? AND source_node_id = ? AND target_node_id = ? AND relation = 'requires'
          LIMIT 1`,
        projectId, rootId, nodeId,
      );
      if (!existingEdge) {
        await run(
          `INSERT INTO graph_edges (id, project_id, source_node_id, target_node_id, relation, sources)
           VALUES (?, ?, ?, ?, 'requires', ?)`,
          generateId('edge'), projectId, rootId, nodeId, null,
        );
      }
    }
  } catch (err) {
