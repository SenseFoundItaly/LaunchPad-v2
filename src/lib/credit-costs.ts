 ...
 *
 * FOUNDER-FACING SURFACE (2026-07-01): only two credit surfaces remain — the
 * monthly-pool badge (chrome.tsx) and the ACTUAL per-message cost shown under
 * each chat turn. Per-action price quotes (skills, applies, doc audits,
 * watchers) were REMOVED, not hidden, to match the "1 message = 1 credit,
 * everything else free" rule — so the old NEXT_PUBLIC_HIDE_CREDITS flag and its
 * HIDE_CREDITS export are gone (any lingering env var is now inert).
 */
