...
const ARCHETYPE_LABEL_KEY: Record<PersonaCardArtifact['archetype'], MessageKey> = {
  customer: 'art.persona.archetype.customer',
  investor: 'art.persona.archetype.investor',
  expert: 'art.persona.archetype.expert',
  competitor: 'art.persona.archetype.competitor',
};
...
export default function PersonaCard({ artifact }: PersonaCardProps) {
  ...
  return (
    ...
    <ArtifactCardShell
      typeLabel={t('art.persona.title')}
      title={artifact.name}
      sources={artifact.sources}
      provenance={artifact.provenance}
    >
      <div className="text-[10px] text-ink-5 mb-1.5">
        {t(ARCHETYPE_LABEL_KEY[artifact.archetype])}
        {artifact.engagement_score !== undefined && ` · ${t('art.persona.engagement')} ${artifact.engagement_score}/10`}
      </div>
      {artifact.demographics && (
      ...
      {hasPlanning && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
          {(artifact.jobs_to_be_done?.length ?? 0) > 0 && (
            <PersonaList label={t('art.persona.jobs')} items={artifact.jobs_to_be_done!} />
          )}
          {(artifact.pains?.length ?? 0) > 0 && (
            <PersonaList label={t('art.persona.pains')} items={artifact.pains!} />
          )}
          {(artifact.channels?.length ?? 0) > 0 && (
            <PersonaList label={t('art.persona.channels')} items={artifact.channels!} />
          )}
        </div>
      )}
