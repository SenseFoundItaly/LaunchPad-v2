...
const CONFIDENCE_LABEL_KEY: Record<NonNullable<MarketSizeTier['confidence']>, MessageKey> = {
  low:    'art.tamsamsom.confidence.low',
  medium: 'art.tamsamsom.confidence.medium',
  high:   'art.tamsamsom.confidence.high',
};
      ...
      <div className="flex items-baseline gap-2">
        ...
        {tier.confidence && (
          <span className="text-[10px] text-ink-5 font-mono">
            {t(CONFIDENCE_LABEL_KEY[tier.confidence])}
          </span>
        )}
      </div>
      {tier.methodology && (
...
export default function TamSamSomCard({ artifact }: TamSamSomCardProps) {
  ...
  return (
    <ArtifactCardShell
      typeLabel={t('art.tamsamsom.title')}
        ...
        <div className="flex-1 space-y-2 min-w-0">
          ...
          {(artifact.timeframe || artifact.market_share_implied) && (
            <div className="text-xs text-ink-4 pt-1 border-t border-line-2">
              {artifact.timeframe && <>{t('art.tamsamsom.timeframe', { value: artifact.timeframe })}</>}
              {artifact.timeframe && artifact.market_share_implied && ' · '}
              {artifact.market_share_implied && <>{t('art.tamsamsom.implied-share', { value: artifact.market_share_implied })}</>}
            </div>
          )}
        </div>
      </div>
