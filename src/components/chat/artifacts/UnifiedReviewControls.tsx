...
const APPLY_LABEL_KEYS: Record<ReviewableLane, MessageKey> = {
  approval: 'common.apply',
  todo: 'art.review.mark-done',
  notification: 'art.review.acknowledge',
};

const REJECT_LABEL_KEYS: Record<ReviewableLane, MessageKey> = {
  approval: 'common.reject',
  todo: 'common.dismiss',
  notification: 'common.dismiss',
};

const APPLIED_LABEL_KEYS: Record<ReviewableLane, MessageKey> = {
  approval: 'art.review.applied',
  todo: 'common.done',
  notification: 'art.review.acknowledged',
};

const REJECTED_LABEL_KEYS: Record<ReviewableLane, MessageKey> = {
  approval: 'art.review.rejected',
  todo: 'common.dismissed',
  notification: 'common.dismissed',
};
    ...
    if (isFooter) {
      return (
        <div className="flex items-center justify-center gap-2 pt-3 mt-3 border-t border-line-2">
          <SpinnerIcon size={16} />
          <span className="text-sm text-ink-4">{t('art.review.processing')}</span>
        </div>
      );
    }
    return (
