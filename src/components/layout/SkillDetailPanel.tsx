      ...
      <div className="w-[520px] bg-surface-sunk border-l border-line flex flex-col h-full overflow-hidden">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {data.status === 'completed' ? (
                <span className="text-xs px-2 py-0.5 rounded-full bg-moss-wash text-moss">{t('layout.skill.completed')}</span>
              ) : (
                <span className="text-xs px-2 py-0.5 rounded-full bg-paper-3 text-ink-4">{t('layout.skill.not-run')}</span>
              )}
              {data.completedAt && (
        ...
        <div className="flex-1 overflow-y-auto">
          {data.status === 'completed' && data.summary ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              ...
              <p className="text-sm text-ink-4 mb-1">{t('layout.skill.not-run-yet')}</p>
              <p className="text-xs text-ink-6 mb-4">{t('layout.skill.start-hint')}</p>
              <Link
                ...
                onClick={onClose}
              >
                {t('layout.skill.run', { name: skill.label })}
              </Link>
            </div>
          )}
        </div>
        ...
        {data.status === 'completed' && (
          <div className="px-5 py-4 border-t border-line shrink-0 space-y-2">
            {nextSteps.length > 0 && (
              <>
                <div className="text-[10px] text-ink-5 uppercase tracking-wider mb-2">{t('layout.skill.next-steps')}</div>
