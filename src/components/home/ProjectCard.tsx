...
export default function ProjectCard({
  ...
  const verdictKey: MessageKey = score >= 8 ? 'homec.verdict-strong-go'
    : score >= 6 ? 'homec.verdict-go'
    : score >= 4 ? 'homec.verdict-caution'
    : 'homec.verdict-not-ready';
    ...
    >
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-ink-6">{t('homec.analyses', { done: skillsCompleted, total: totalSkills })}</span>
      ...
      <div className="flex items-center justify-between text-[10px] text-ink-6">
        <span>
          {weeklyAlerts > 0
            ? (weeklyAlerts === 1 ? t('homec.alerts-week-one') : t('homec.alerts-week-many', { n: weeklyAlerts }))
            : t('homec.no-alerts')}
        </span>
